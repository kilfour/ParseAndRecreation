...
public class EvalTests
{
    ...
    // --- Sum level: + and - (left-associative) ---
    [Fact]
    ...
    public void SubtractionLeftAssociative() =>
        Assert.Equal(-5, LostIn.Translation("2-3-4").Eval()); // (2-3)-4

    [Fact]
    public void MixAddSub() =>
        Assert.Equal(0, LostIn.Translation("10-7-3+0").Eval()); // ((10-7)-3)+0

    // --- Prod level: * and / (left-associative) ---
    [Fact]
    ...
    public void DivisionLeftAssociativeChain() =>
        Assert.Equal(1, LostIn.Translation("8/4/2").Eval()); // (8/4)/2

    [Fact]
    public void DivisionThenMultiplicationLeftAssociative() =>
        Assert.Equal(9, LostIn.Translation("6/2*3").Eval()); // (6/2)*3

    // --- Pow level: ^ (right-associative) ---
    [Fact]
    ...
    public void PowerRightAssociative() =>
        Assert.Equal(512, LostIn.Translation("2^3^2").Eval()); // 2^(3^2)
    ...
    public void DoubleUnaryMinus() =>
        Assert.Equal(1, LostIn.Translation("--1").Eval()); // -(-1)

    [Fact]
    public void UnaryMinusBindsTighterThanSum() =>
        Assert.Equal(2, LostIn.Translation("3+-1").Eval()); // 3 + (-1)

    [Fact]
    public void BinaryMinusFollowedByUnaryMinus() =>
        Assert.Equal(2, LostIn.Translation("1 - -1").Eval()); // 1 - (-1)

    // --- Precedence checks across levels ---
    [Fact]
    public void MulBeatsAdd() =>
        Assert.Equal(7, LostIn.Translation("1+2*3").Eval()); // 1+(2*3)

    [Fact]
    public void PowBeatsMulBeatsAdd() =>
        Assert.Equal(19, LostIn.Translation("1+2*3^2").Eval()); // 1 + 2*(3^2)

    [Fact]
    public void ParenthesesOverridePrecedence() =>
        Assert.Equal(27, LostIn.Translation("(1+2)*3^2").Eval()); // (1+2)*(3^2)

    // --- Unary minus with powers (assignment rule: -(x^2), not (-x)^2) ---
    [Fact]
    public void UnaryMinusVsPower() =>
        Assert.Equal(-4, LostIn.Translation("-2^2").Eval()); // -(2^2)

    [Fact]
    public void ParenthesizedNegativeBaseToPower() =>
        Assert.Equal(4, LostIn.Translation("(-2)^2").Eval());

    // --- A couple of decimal combos that land on integers to avoid precision fuss ---
    [Fact]
    public void DecimalTimesInteger() =>
        Assert.Equal(6, LostIn.Translation("1.5*4").Eval()); // 6
